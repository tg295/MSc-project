...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 nav1p9 C:/Users/theog/OneDrive/Desktop/ICL/Thesis/Final models/Unmyelinated with LDIFF5/nav1p9.mod\n");
...
double alpham ( _threadargsprotocomma_ double _lv ) {
   double _lalpham;
 _lalpham = 1.032 / ( 1.0 + exp ( ( _lv + 6.99 ) / ( - 14.87115 ) ) ) ;
...
double alphas ( _threadargsprotocomma_ double _lv ) {
   double _lalphas;
 _lalphas = 0.00000016 * exp ( - _lv / 12.0 ) ;
...
double alphah ( _threadargsprotocomma_ double _lv ) {
   double _lalphah;
 _lalphah = 0.06435 / ( 1.0 + exp ( ( _lv + 73.26415 ) / 3.71928 ) ) ;
...
double betam ( _threadargsprotocomma_ double _lv ) {
   double _lbetam;
 _lbetam = 5.79 / ( 1.0 + exp ( ( _lv + 130.4 ) / 22.9 ) ) ;
...
double betas ( _threadargsprotocomma_ double _lv ) {
   double _lbetas;
 _lbetas = 0.0005 / ( 1.0 + exp ( - ( _lv + 32.0 ) / 23.0 ) ) ;
...
double betah ( _threadargsprotocomma_ double _lv ) {
   double _lbetah;
 _lbetah = 0.13496 / ( 1.0 + exp ( ( _lv + 10.27853 ) / ( - 9.09334 ) ) ) ;
...
double vtrap ( _threadargsprotocomma_ double _lx , double _ly ) {
 ...
 if ( fabs ( _lx / _ly ) < 1e-6 ) {
     _lvtrap = ( _ly / _lx ) / ( - 1.0 - ( _lx / 2.0 * _ly ) ) ;
     }
   else {
     _lvtrap = 1.0 / ( 1.0 - exp ( _lx / _ly ) ) ;
     }
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
  ...
  s = s0;
 {
   ...
   s = alphas ( _threadargscomma_ v ) / ( alphas ( _threadargscomma_ v ) + betas ( _threadargscomma_ v ) ) ;
   }
