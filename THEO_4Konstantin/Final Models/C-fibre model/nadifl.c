...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "diam");
 	...
 	ivoc_help("help ?1 nadifl C:/Users/theog/OneDrive/Desktop/ICL/Thesis/Final models/Unmyelinated with Ion diffusion/nadifl.mod\n");
...
_MATELM1(0, 0) *= ( PI * diam * diam / 4.0);  }
 ...
 f_flux = b_flux = 0.;
 _RHS1( 0) += (b_flux =   ( - ina / ( FARADAY ) * PI * diam * ( 1e4 ) ) );
 /*FLUX*/
    } return _reset;
 }
 ...
 */
 /* ~ nai < < ( - ina / ( FARADAY ) * PI * diam * ( 1e4 ) )*/
 f_flux = b_flux = 0.;
 Dnai += (b_flux =   ( - ina / ( FARADAY ) * PI * diam * ( 1e4 ) ) );
 /*FLUX*/
...
static double _difcoef1(int _i, double* _p, Datum* _ppvar, double* _pdvol, double* _pdfcdc, Datum* _thread, _NrnThread* _nt) {
 ...
 if (_i == 0) {
  *_pdfcdc = nrn_nernst_coef(_type_ina)*( ( - _ion_dinadv  / ( FARADAY ) * PI * diam * ( 1e4 ) ));
 }else{ *_pdfcdc=0.;}
