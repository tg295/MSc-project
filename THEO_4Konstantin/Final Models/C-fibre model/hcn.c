...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 6, "cvodeieq");
 	...
 	ivoc_help("help ?1 hcn C:/Users/theog/OneDrive/Desktop/ICL/Thesis/Final models/Unmyelinated with LDIFF5/hcn.mod\n");
...
/*CVODE*/
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   rates ( _threadargscomma_ v ) ;
    ns = ns + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_ns)))*(- ( ( ( ninf ) ) / tau_ns ) / ( ( ( ( - 1.0 ) ) ) / tau_ns ) - ns) ;
    nf = nf + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_nf)))*(- ( ( ( ninf ) ) / tau_nf ) / ( ( ( ( - 1.0 ) ) ) / tau_nf ) - nf) ;
   }
  return 0;
}
...
double rates ( _threadargsprotocomma_ double _lv ) {
   double _lrates;
 ninf = 1.0 / ( 1.0 + exp ( ( _lv + 87.2 ) / 9.7 ) ) ;
   if ( _lv > - 70.0 ) {
     tau_ns = 300.0 + 542.0 * exp ( ( _lv + 25.0 ) / 20.0 ) * q10h ;
     }
   if ( _lv < - 70.0 ) {
     tau_ns = 2500.0 + 100.0 * exp ( ( _lv + 240.0 ) / 50.0 ) * q10h ;
     }
   if ( _lv > - 70.0 ) {
     tau_nf = 140.0 + 50.0 * exp ( ( _lv + 25.0 ) / ( - 20.0 ) ) * q10h ;
     }
   if ( _lv < - 70.0 ) {
     tau_nf = 250.0 + 12.0 * exp ( ( _lv + 240.0 ) / 50.0 ) * q10h ;
     }
